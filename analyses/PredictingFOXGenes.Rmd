...
```{r}
df <- read_csv(file.path("data", "Anabaena7120.csv"))

df$Annotation <- df$`Locus Name`
fox1 <- read_csv(file.path("data", "FoxGenePositives.csv"))
fox1 <- fox1 %>% mutate(Annotation = GENES)

df_tree2 <- df #as.data.frame(cbind(df, kmer, kmer2, kmer4))

negative <- read_csv(file.path("data", "FoxGeneNegatives.csv"))
negative$Annotation <- negative$GENES

unlabeled <- read_csv(file.path("data", "UnlabeledFoxGenes.csv"))
...
write_csv(df_tree3, file.path("results", "Predictions.csv"))
